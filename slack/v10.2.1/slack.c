...
{
    pxArray[ 0 ] = 0; // xTickCount;
    pxArray[ 1 ] = 0; // getSsTCB( pxCurrentTCB )->xId; //getSsTCB( pxCurrentTCB )->xCur;
    pxArray[ 2 ] = 0; // xSlackSD;
    ...
    while( listGET_END_MARKER( &( xSsTaskList ) ) != pxTaskListItem )
    {
        pxArray[ xI ] = 0; //getSsTCB( listGET_LIST_ITEM_OWNER( pxTaskListItem ) )->xSlack;
        xI = xI + 1;
        pxTaskListItem = listGET_NEXT( pxTaskListItem );
    }
}
